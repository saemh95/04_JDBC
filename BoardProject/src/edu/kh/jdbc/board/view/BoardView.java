...
import edu.kh.jdbc.board.model.BoardService;
...
public class BoardView {
	...
	public void selectBoard() {
		...
		try {
			...
			if(!board.getCommentList().isEmpty()) {
				for(Comment c : board.getCommentList()) {
					System.out.printf("[댓글 번호 : %d] 작성자 : %s 작성일 : %s \n %s \n",
								c.getCommentNo(), c.getMemberName(), c.getCreateDate(),
								c.getCommentContent()
							);
				System.out.println("-------------------------------------------------");
				}
			...
			commentView.commentMenu(input); //여기서 input 은 게시글 번호
...
//					수정/삭제 기능 수행 후 게시판 메뉴로 돌아가기
					switch(input) {
					case 1 : updateBoard(board.getBoardNo()); 
						return;
					case 2: deleteBoard(board.getBoardNo());
						return;
					case 0: return;
					default: System.out.println("\n***잘못 입력 하셨습니다***\n");
					}
	...
	private void deleteBoard(int boardNo) {
		...
		try {
			...
			int result = boardService.deleteBoard(boardNo, Session.loginMember.getMemberNo());
